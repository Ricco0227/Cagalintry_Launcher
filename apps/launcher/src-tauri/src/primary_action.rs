// Wired to real instance state in Phase 3; the rules and their tests are here
// first because they are the contract the rest of the UI is built against.
#![allow(dead_code)]

//! What the one big button on an instance says and does.
//!
//! Derived state, never a stored flag — the button is recomputed from the
//! instance's current facts every time they change. That is what makes it flip
...
/// store, the running-process table, and the instance's sync state.
...
pub struct InstanceStatus {
    ...
    /// An install or update is in flight for this instance.
    pub busy: bool,
    /// The game is already running for this instance.
    pub running: bool,
    /// Revision currently on disk, if this instance is bound to a synced pack.
    ...
    /// Content has never been installed for this instance.
...
pub fn resolve(status: InstanceStatus) -> PrimaryAction {
...
mod tests {
    ...
    /// A linked, installed, idle instance — the ordinary case.
    fn ready() -> InstanceStatus {
        InstanceStatus {
    ...
    fn without_a_linked_account_the_button_offers_linking() {
        let status = InstanceStatus { has_linked_minecraft_account: false, ..ready() };
    ...
    #[test]
    fn a_local_instance_with_no_pack_just_plays() {
    ...
    fn work_in_progress_disables_the_button() {
        let busy = InstanceStatus { busy: true, ..ready() };
        assert_eq!(resolve(busy), PrimaryAction::Busy);
        assert!(resolve(busy).is_disabled());

        let running = InstanceStatus { running: true, ..ready() };
    ...
    fn a_never_installed_pack_offers_install() {
        let status = InstanceStatus { needs_install: true, ..ready() };
